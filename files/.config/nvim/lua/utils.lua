...
function M.map(keymaps)
    for _, map in pairs(keymaps) do vim.cmd(M.t(map)) end
end
...
function M.substitute() 
  vim.fn['tools#buffer_substitute']()
end
...
function M.terminal() 
  vim.fn['utils#toggle']('term', 'term#open')
end
...
function M.quickfix() 
  vim.fn['utils#toggle']('qf', 'copen')
end

function M.loclist() 
  vim.fn['utils#toggle']('qf', 'lopen')
end
