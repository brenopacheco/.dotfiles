...
function M.argadd() print('todo') end
function M.argdelete() print('todo') end
function M.args() print('todo') end
...
function M.files(monorepo)
    ...
    if is_git then
        if monorepo then
          require('telescope.builtin').find_files({cwd = M.rootnpm(), hidden = true})
        else
