...
function M.argadd()
	...
	vim.cmd([[argadd]])
	print('Added "' .. vim.fn.bufname() .. '" to arglist: ' .. vim.inspect(vim.fn.argv()))
end

function M.argdelete()
	if vim.fn.argc() > 0 then
		vim.cmd("silent! argdelete " .. vim.fn.bufname())
		print('Removed "' .. vim.fn.bufname() .. '" to arglist: ' .. vim.inspect(vim.fn.argv()))
	else
...
function M.cnext()
	vim.fn["quickfix#next"]()
end
function M.colder()
	vim.fn["quickfix#colder"]()
end
function M.cprev()
	vim.fn["quickfix#prev"]()
end
...
function M.diff()
	vim.cmd([[Gvdiffsplit | norm! \<c-w>x]])
end
...
function M.fugitive()
	vim.fn["utils#toggle"]("fugitive", "G")
end
...
function M.insert_line_above()
	vim.cmd([[norm mzO<C-[>0D`z]])
end
function M.insert_line_below()
	vim.cmd([[norm mzo<C-[>0D`z]])
end
function M.lf()
	vim.cmd("Lf")
...
function M.live_grep()
	require("telescope.builtin").live_grep({ cwd = M.root(), hidden = true })
end
function M.lnext()
	vim.fn.lnext()
end
function M.loclist()
	vim.fn["utils#toggle"]("qf", "lopen | wincmd p")
end
function M.lprev()
	vim.fn.lprev()
end
...
function M.get_url()
	...
	for i, match, l in uri:gmatch("()([A-Za-z0-9_.\\-~:/?=]+)()") do
		if col >= i and col < l then
			if not string.match(match, "^https?://") and not string.match(match, "^www\\.") then
				match = "https://" .. match
			end
			return match
		end
...
function M.open_url()
	vim.cmd("!chromium " .. M.get_url())
end
function M.preview_hunk()
	require("gitsigns").preview_hunk()
end
function M.projects()
	require("util.picker").projects()
end
function M.git_dirs()
	require("util.picker").git_dirs()
end
function M.qf_global()
	vim.fn["quickfix#filter"](vim.fn.input("> /"))
end
function M.quickfix()
	vim.fn["utils#toggle"]("qf", "copen | wincmd p")
end
...
function M.substitute()
	vim.fn["substitute#buffer_substitute"]()
end
function M.tagbar()
