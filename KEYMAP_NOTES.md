...
### Misc
gm    make                    run :make - or open up a :Make <command> prompt
...
I might be missing quite useful commands:
- files (local, root?) - unless we rely on smart for everything?
  if that's the case, we need a smart picker that allows us to toggle
  the source and the root path
- should we use g<key> for goto, and something else for actions?
